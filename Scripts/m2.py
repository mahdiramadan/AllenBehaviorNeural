...
def run_svm(final_data, y_train):
    ...
    t1 = time.time()
    clf = joblib.load('clf_final_all_beh.pkl')
    print('done loading')
    y_true, y_pred = y_test, clf.predict(X_test)
    print('done predicting')
    joblib.dump(y_pred, 'y_pred_497256116_all_beh.pkl', compress = 1)
...
def get_data(ep, lims_ID):
    ...
    y_train = []
    dimension = 260*540
    wheel = joblib.load('dxds'+ str(lims_ID)+'.pkl')
    ...
    data = np.array(group.get('features'))[k:50000]

    for item in range(k, 50000):
