...
def run (ids , ann_data):
    ...
    results_A = [[] for _ in range(len(stimuli))]
    results_B = [[] for _ in range(len(stimuli))]
    results_C = [[] for _ in range(len(stimuli))]
    ...
    for id in ids:
        ...
        if status is not None and 'published' in status:
            ...
            for stim in stimuli:
                ...
                try:
                    if ((a/float(np.sum(t_fid)))*100) <= 100:
                        results_fid[int(stimuli.index(stim))].append((a/float(np.sum(t_fid)))*100)
                    else:
                ...
                try:
                    if ((b/ float(np.sum(t_neither))) * 100) <= 100:
                        results_neither[int(stimuli.index(stim))].append((b / float(np.sum(t_neither))) * 100)
                    else:
                ...
                try:
                    if ((c/ float(np.sum(t_mov))) * 100) <= 100:
                        results_mov[int(stimuli.index(stim))].append((c / float(np.sum(t_mov))) * 100)
                    else:
    ...
    for stim in stimuli:
        ...
        plt.title('Distribution of Behavior Frequency During '  + str(stim) +' Stimuli')
        plt.show()
