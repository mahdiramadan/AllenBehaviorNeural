...
#                    tmp[xy_coords[0], xy_coords[1], 2] = 255
    ...
    def video_annotation(self):
        ...
        file_name = self.rb.get_file_string()

        wheel = joblib.load('C:\Users\mahdir\Desktop\Mahdi files\Wheel\dxds'+ str(self.lims_ID)+'.pkl')
        first_non_nan = next(x for x in wheel if not isnan(x))
        first_index = np.where(wheel == first_non_nan)[0]

        pred = joblib.load('y_neither_movement_fidget_0585.pkl')
        ...
        out = cv2.VideoWriter('fid_move_nomove_nowheel_60000_80000.avi', fourcc, fps, (frameWidth, frameHeight))
        data_pointer.set(1, 60000+first_index)
